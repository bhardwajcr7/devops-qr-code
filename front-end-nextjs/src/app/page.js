...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
//      const response = await axios.post(`http://${window.location.hostname}:8000/generate-qr/?url=${url}`);
      const response = await axios.post(`http://be-service/generate-qr/?url=${url}`);
